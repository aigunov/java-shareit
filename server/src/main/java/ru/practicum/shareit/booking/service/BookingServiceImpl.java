...
public class BookingServiceImpl implements BookingService {
    ...
    @Transactional(readOnly = true)
    @Override
    public List<BookingResponse> findUserBookingByStatus(long userId, Status status) {
        userRepository.findById(userId)
                .orElseThrow(() -> new NoSuchElementException("User with id " + userId + " not found"));
        List<BookingResponse> bookingResponseList = switch (status) {
        ...
        log.info("Find user booking by Status \"{}\" is {}", status, bookingResponseList);
        return bookingResponseList;
    }

    @Override
    public List<BookingResponse> findOwnerBookingByStatus(long userId, Status status) {
        userRepository.findById(userId)
                .orElseThrow(() -> new NoSuchElementException("User with id " + userId + " not found"));
        List<BookingResponse> bookingResponseList = switch (status) {
        ...
        log.info("Find owner booking by Status \"{}\" is {}", status, bookingResponseList);
        return bookingResponseList;
    }
}
